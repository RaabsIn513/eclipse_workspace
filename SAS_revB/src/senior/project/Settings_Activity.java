	...
	public boolean loadSettingsFile()
	{
		...
		try{
			...
			if( vals.length == 0)
			{
				...
				edSet = SettingsFile.edit();
				edSet.putInt("SAMPLE_RATE", 8000);
				edSet.putInt("SAMP_HISTORY_SIZE", 64);
				edSet.putInt("AMP_HIST_SIZE", 32);
				edSet.putInt("THRESH_DIFF", 50);		// (current_max_amp) - (avg_max_amp)
				edSet.putInt("THRESH_SAMPS", 16);		// The number of consecutive samples where 
